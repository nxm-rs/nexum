...
use crate::namespaces::{eth, net, web3, wallet};
...
) -> impl Fn(Params<'static>, Arc<()>, jsonrpsee::Extensions) -> BoxFuture<'static, RpcResult<Value>> + Send + Sync + Clone + 'static {
    let method_name: Arc<String> = Arc::new(method_name.to_string());
    let shared_client = Arc::clone(&shared_client);

    move |params: Params<'static>, _: Arc<()>, _: jsonrpsee::Extensions| -> BoxFuture<'static, RpcResult<Value>> {
        ...
        trace!("Received request: {} with params: {:?}", *method_name, params);
